...
contract PerformUpkeep is Script {
    function performUpkeep(address raffle) public {
        console.log("Performing upkeep on raffle:", raffle);
        vm.broadcast();
        Raffle(raffle).performUpkeep("");
    }

    function performUpkeepUsingConfig() public {
        address raffle = DevOpsTools.get_most_recent_deployment("Raffle", block.chainid);
        performUpkeep(raffle);
    }

    function run() external {
        performUpkeepUsingConfig();
    }
}